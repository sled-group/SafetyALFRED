"""
Qwen2.5-VL Few-shot ICL Evaluation using vLLM for fast inference
...
def main():
    ...
    parser.add_argument('--no-examples', action='store_true', help='Run in zero-shot mode without any examples')
    ...
    if args.no_examples:
        args.num_examples = 0
        print("Running in zero-shot mode (no examples)")
    ...
    # Skip category filtering for generated-mode only since generated trajectories don't have category metadata
    if args.generated_mode == 'only':
        print(f"\nSkipping category filtering for generated-mode only (generated trajectories don't have category metadata)", flush=True)
        filtered_test_indices = test_indices
    else:
    ...
    if args.num_examples == 0:
        print(f"\nZero-shot mode enabled - skipping example preparation", flush=True)
    else:
    ...
    if args.num_examples > 0:
        ...
        # Zero-shot mode: create empty configs for each category
        test_categories = sorted([k for k in category_trajectory_groups.keys() if k is not None])
