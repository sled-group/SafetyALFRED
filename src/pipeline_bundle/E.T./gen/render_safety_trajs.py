...
def cfg_args():
    ...
    data_output = 'generated_safety_2.1.0'
...
def start_worker(worker_id, raj_list, args, lock, processed_files_path):
    '''
    worker loop
    '''
    ...
    for partition in args.partitions:
        for dir_name in sorted(glob.glob(os.path.join(args.data_input, partition, '*/*'))):
            ...
            for file_name in os.listdir(dir_name):
                # if 'trial_T20190908_231649_805205' in os.path.basename(dir_name):
                if  "pick_two_obj_and_place" not in dir_name:# and "Sliced" not in dir_name:
                    # print("FILE_NAME:", file_name)
                    if "_safety_traj_" in file_name and ("fall" in file_name or "spoil" in file_name):
