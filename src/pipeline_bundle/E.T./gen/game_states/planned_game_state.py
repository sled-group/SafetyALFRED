...
'''
        ...
        # sliceable objects (only if object_sliced is true and it's the target object)
        sliceable_str = '\n        '.join(['(sliceable %s)' % obj for obj in objects
                                          if (constants.data_dict['pddl_params']['object_sliced'] and
                                              self.object_target is not None and
                                              object_dict[obj]['objectType'] == constants.OBJECTS[self.object_target] and
                                              object_dict[obj]['objectType'] in constants.VAL_ACTION_OBJECTS['Sliceable'])])
