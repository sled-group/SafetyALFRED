...
def main(args):
    ...
    processed_files_path = os.path.join(args.data_output, 'processed_metadata.txt')
    ...
    # make a list of all the traj_data json files from processed.txt
    traj_list = []
    processed_txt_path = os.path.join(args.data_output, 'processed.txt')
    
    if not os.path.exists(processed_txt_path):
        print(colored(f'processed.txt not found at {processed_txt_path}', 'red'))
        return
    
    print(f'Reading trajectories from processed.txt: {processed_txt_path}')
    with open(processed_txt_path, 'r') as f:
        for line in f:
            ...
            # Parse line format: path/to/json;status
            parts = line.split(';')
            if len(parts) != 2:
                continue
                
            json_path_rel, status = parts
            if status == '1':  # Only process successful entries
                # Check if the json file exists in the input data
                full_json_path = os.path.join(args.data_input, json_path_rel)
                if os.path.isfile(full_json_path):
                    # Check if it's in the desired partitions
                    partition = json_path_rel.split('/')[0]
                    if partition in args.partitions:
                        traj_list.append(json_path_rel)
