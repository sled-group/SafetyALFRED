...
class TaskGameState(PlannedGameState):
    ...
    def get_goal_pddl(self):
        ...
        # NOTE: Do NOT append "Sliced" to the object type for slicing goals
        # The SliceObject PDDL action sets isSliced predicate but does NOT change object type
        # So a sliced Bread is still BreadType (not BreadSlicedType), just with isSliced=true
        obj_name = constants.OBJECTS[self.object_target] if self.object_target is not None else ""

        goal_str = goal_str.format(obj=obj_name,
    ...
    def setup_problem(self, seed=None, info=None, scene=None, objs=None):
        ...
        if constants.FORCED_SAMPLING or objs is None:
            ...
            if len(chosen_class_available) == 0:
                print("Task Failed - %s" % constants.pddl_goal_type)
                print("DEBUG: Looking for parent object type: '%s'" % constants.OBJ_PARENTS[objs['pickup']])
                print("DEBUG: Available pickupable object types in scene:")
                for obj in pickupable_objects:
                    print(f"  - {obj['objectType']} (objectId: {obj['objectId']})")
                print("DEBUG: ALL objects in scene (including non-pickupable):")
                for obj in self.env.last_event.metadata['objects']:
                    print(f"  - {obj['objectType']} (objectId: {obj['objectId']}, pickupable: {obj.get('pickupable', False)})")
        ...
        # Only set object_sliced if not already set (e.g., from trajectory data)
        # When replaying trajectories, we want to preserve the original object_sliced value
        if 'object_sliced' not in constants.data_dict['pddl_params']:
