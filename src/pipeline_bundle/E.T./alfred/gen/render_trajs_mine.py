...
from alfred.env.thor_env_original import ThorEnv
...
def augment_traj(env, json_file, args, video_saver, render_settings):
    ...
    match = re.search('\d+', traj_data['scene']['floor_plan'])
    if match:
    ...
    if match:
        ...
        if number >= 1 and number <= 30:
            ...
            env.step(dict(action="GetReachablePositions"))
            reachable_positions = env.last_event.metadata["actionReturn"]
            objects_dict = {
                obj["name"]: {k: v for k, v in obj.items() if k != "name"}
                for obj in objects
            }
            task_objects[traj_data["task_id"]] = (objects_dict, reachable_positions)
            # print(task_objects)
            return -1
            count += 1
    return 0
...
def start_worker(worker_id, traj_list, args, lock, processed_files_path):
    ...
    while len(traj_list) > 0:
        ...
        lock.release()
    with open("kitchen_tasks.pkl", "wb") as f:
        pickle.dump(task_objects, f)
    print(count)
    env.stop()
    print("Finished.")
